...
def training_loop(
    ...
    minibatch_repeats       = 4,        # Number of minibatches to run before adjusting training parameters. WAS 4 - l4r
    lazy_regularization     = True,     # Perform regularization as a separate training step?
    G_reg_interval          = 4,        # How often the perform regularization for G? Ignored if lazy_regularization=False.
    D_reg_interval          = 16,       # How often the perform regularization for D? Ignored if lazy_regularization=False.
    reset_opt_for_new_lod   = True,     # Reset optimizer internal state (e.g. Adam moments) when new layers are introduced?
    total_kimg              = 35000,    # Total length of the training, measured in thousands of real images.
    ...
    network_snapshot_ticks  = 10,       # How often to save network snapshots? None = only save 'networks-final.p1kl'.
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_tick < 0 or cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Save snapshots.
            # l4rz nah, doesnt work with attn
            #if image_snapshot_ticks is not None and (cur_tick % image_snapshot_ticks == 0 or done):
                # comment out to disable grid saving -l4rz
                grid_fakes = Gs.run(grid_latents, grid_labels, is_validation=True, minibatch_size=sched.minibatch_gpu)
